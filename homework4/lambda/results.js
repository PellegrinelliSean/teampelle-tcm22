...
exports.handler = async (event) => {
  ...
  xml2js.parseString(nodes, (err, result) => {
    ...
    result.ClassResult.PersonResult.forEach(function(elem){
      ...
      temp.Orgid = elem.Organisation[0].Id[0];
      temp.Org = elem.Organisation[0].Name[0];
      temp.Country = elem.Organisation[0].Country[0].$.code;
      temp.Status = elem.Result[0].Status[0];
      if (elem.Result[0].hasOwnProperty('Position'))
        temp.Position = elem.Result[0].Position[0];
      if (elem.Result[0].hasOwnProperty('StartTime'))
        temp.Start = elem.Result[0].StartTime[0].substring(11,19);
      if (elem.Result[0].hasOwnProperty('FinishTime'))
        temp.End = elem.Result[0].FinishTime[0].substring(11,19);
      if (elem.Result[0].hasOwnProperty('Time'))
        temp.Time = new Date(elem.Result[0].Time[0] * 1000).toISOString().substr(11, 8);
