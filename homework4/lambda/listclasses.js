...
exports.handler = async (event) => {
  ...
  var nodes = select('//ns:Class', doc);
  nodes = '<Classes>' + nodes + '</Classes>';
  var Event = select('//ns:Event', doc);
  
  var list = new Object();
  list.race_name;
  list.race_date;
  xml2js.parseString(Event, (err, result) => {
    if (err)
      throw err
    list.race_name = result.Event.Name[0];
    list.race_date = result.Event.StartTime[0].Date[0];
  });
  ...
  xml2js.parseString(nodes, (err, result) => {
    if (err)
      throw err;
    result.Classes.Class.forEach(function(elem){
      var temp = new Object();
      temp.name = elem.Name[0];
      temp.id = elem.Id[0];
      list.race_classes.push(temp);
      list.count_classes++;
    });
  });
